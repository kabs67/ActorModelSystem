        ...
        {callTaxi, [Username,X,Y,X1,Y1]} ->
          ...
          {cancelUser,[Username]} ->
                                          ...
                                          GestorDb ! {sabertempo,Username,self()},
                                           ...
                                           Preco = preco(Time3,0,GestorOk,Username),
                                          ...
                                          receive
                                             {pagarmetade,Tempo} ->
                                              if Tempo < 60.0 ->
                                                gen_tcp:send(Socket,"canceled  demorou_" ++ U ++ "_o_preco_a_pagar_" ++ "0.0" ++ "\n");
                                              true ->
                                      ...
                                      receive
                                            ...
                                            {chegoutaxi,Tempo,Preco} ->
                                                   ...
                                                   true ->
                                                    ...
                                                    K = float_to_list(Tempo,[{decimals, 2}]),
                                                    gen_tcp:send(Socket,"canceled  demorou_" ++ K ++ "_o_preco_a_pagar_" ++ Z ++ "\n")
                                                  end;
...
preco(Tempo,X,GestorOk,Username) ->
  %if X == 0 ->
  %  GestorOk ! {saberflags,Username,self()},
  %  receive
  %        {aindanaotaxi} ->
  %                 if Tempo < 60.0 -> 0.0;
  %                 true -> Tempo*2.0
  %                 end;
  %        {chegoutaxi} -> Tempo*2.0      
  %  end;
 %true -> 
 Tempo*2.0.
%end.
